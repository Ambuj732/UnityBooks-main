...
exports.login = (req, res) => {
	...
	if (!email || !password) errorMsg(res, null, "Please Enter Your Email and Password", 400, null);

	db.query("select * from USERS U JOIN SELLER S ON U.UID = S.SID where U.EMAIL=?", [email], (error, result) => {
		if (error) errorMsg(res, error, "Internal Server Error", 500, null);
		if (result.length <= 0) errorMsg(res, null, "Wrong Email Address", 400, null);
		const pass = result[0].PASSWORD;
		const id = parseInt(result[0].UID);
		const name = result[0].SELLER_NAME;
		const type = parseInt(result[0].TYPE);
		if (type === 0 || type === null) errorMsg(res, null, "Seller Account not Available", 400, id);
		else {
			bcrypt.compare(password, pass).then((passwordMatch) => {
				if (!passwordMatch) errorMsg(res, null, "Wrong Password", 400, id);
				else {
					const token = jwt.sign({ id: id, name: name, type: type }, process.env.JWT_SECRET,
						{ expiresIn: process.env.JWT_EXPIRES_IN }
					);

					try {
						db.query("UPDATE USERS SET LOGIN = CURRENT_TIMESTAMP WHERE UID=?"
							, [id], (error, result) => { if (error) errorMsg(res, error, "Internal Server Error - Failed to update login", 500, id); })
					}
					catch (error) { errorMsg(res, error, "Internal Server Error - Failed to update login", 500, id); }
			...
			}).catch((error) => { errorMsg(res, error, "Internal Server Error - Failed to compare password", 500, id); });
		}
	});
};
...
exports.isLoggedIn = async (req, res, next) => {
	if (req.cookies.admin) {
		try {
			const decode = await promisify(jwt.verify)(req.cookies.admin, process.env.JWT_SECRET);
			const sid = parseInt(decode.id);
			const type = parseInt(decode.type);
			db.query("SELECT * from USERS WHERE UID=?", [sid], async (err, result) => {
...
exports.order = async (req, res) => {
	const sid = await getUID(req, res);
	db.query("select * from ORDERS where SELLER=? ORDER BY OID DESC", [sid], async (error, orders) => {
...
exports.products = async (req, res) => {
	try {
		...
		const result = await new Promise((resolve, reject) => {
			db.query("SELECT * FROM INVENTORY I JOIN BOOKS B ON I.BID = B.BID ORDER BY I.PID DESC;", (error, result) => {
	...
	} catch (error) { errorMsg(res, error, "Internal Server Error", 500, sid) };
};
...
exports.product = async (req, res) => {
	...
	try {
		const sid = await getUID(req, res);
		const result = await new Promise((resolve, reject) => {
			db.query("SELECT * FROM INVENTORY I JOIN BOOKS B ON I.BID = B.BID WHERE I.PID = ?;", [pid], async (error, result) => {
				if (error) reject(error);
				else {
					try {
						const categories = await new Promise((resolveCategories, rejectCategories) => {
							db.query("SELECT C.* FROM PROD_CAT PC JOIN CATEGORY C ON PC.CATID = C.CATID WHERE PC.BID = ?;"
								, [result[0].BID], (categoriesError, categoriesResult) => {
					...
					} catch (categoriesError) { reject(categoriesError); }
				}
			});
		});

		if (result.product.length <= 0) errorMsg(res, null, "No Products Found", 404, sid);
		else {
			return res.status(200).json({
				success: true,
...
exports.editProduct = async (req, res) => {
	let { pid, name, isbn, author, format, language, condition, mrp, cp, sp, discount, quantity, pages, weight, description, category, bid, img } = req.body;
	const sid = await getUID(req, res);
	try {
		// Update the product details using promise
		const products = await new Promise((resolve, reject) => {
			db.query("UPDATE INVENTORY SET COND = ?, QTY = ?, CP = ?, SP = ?, DISCOUNT = ?, LANG = ? WHERE PID = ?;"
				, [condition, quantity, cp, sp, discount, language, pid], async (error, result) => {
					if (error) {
						errorMsg(res, error, "Internal Server Error - Update INVENTORY", 500, sid);
						reject(error);
					}
					else {
						try {
							db.query("UPDATE BOOKS SET NAME = ?, ISBN = ?, AUTHOR = ?, FORMAT = ?, PAGES = ?, WEIGHT = ?, DESCRIPTION = ?, MRP = ?, IMG = ? WHERE BID = ?;"
								, [name, isbn, author, format, pages, weight, description, mrp, img, bid], async (error1, result1) => {
									if (error1) {
										errorMsg(res, error1, "Internal Server Error - Update BOOKS", 500, sid);
										reject(error1);
									}
		...
		const categories = await new Promise((resolveCategories, rejectCategories) => {
			db.query("SELECT C.* FROM PROD_CAT PC JOIN CATEGORY C ON PC.CATID = C.CATID WHERE PC.BID = ?;"
				, [bid], (categoriesError, categoriesResult) => {
					if (categoriesError) rejectCategories(categoriesError);
					else resolveCategories(categoriesResult);
				});
		});
		...
		const addedCategories = category.filter((newCategory) => !currentCategories.includes(newCategory));

		// Find removed categories
		const removedCategories = currentCategories.filter((currentCategory) => !category.includes(currentCategory));
		...
		for (const removedCategory of removedCategories) {
			await new Promise((resolveRemove, rejectRemove) => {
				db.query("DELETE FROM PROD_CAT WHERE BID = ? AND CATID = (SELECT CATID FROM CATEGORY WHERE NAME = ?);",
					[bid, removedCategory], (removeError, removeResult) => {
		...
		for (const addedCategory of addedCategories) {
			await new Promise((resolveAdd, rejectAdd) => {
				db.query("INSERT INTO PROD_CAT (BID, CATID) VALUES (?, (SELECT CATID FROM CATEGORY WHERE NAME = ?));",
					[bid, addedCategory], (addError, addResult) => {
...
exports.addProduct = async (req, res) => {
	let { name, isbn, author, format, language, condition, mrp, cp, sp, discount, quantity, pages, weight, description, category, img } = req.body;
	const sid = await getUID(req, res);
	...
	try {
		...
		const existingBook = await new Promise((resolve, reject) => {
			db.query('SELECT BID FROM BOOKS WHERE ISBN = ? AND FORMAT = ? LIMIT 1;', [isbn, format], (error, result) => {
				if (error) reject(error);
		...
		if (existingBook) bid = existingBook.BID;
			...
			bid = await new Promise(async (resolve, reject) => {
				db.query('INSERT INTO BOOKS (ISBN, NAME, MRP, DESCRIPTION, AUTHOR, FORMAT, PAGES, WEIGHT, REVIEW, IMG) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);'
					, [isbn, name, mrp, description, author, format, pages, weight, review, img], async (error, result) => {
						if (error) {
							errorMsg(res, error, "Internal Server Error - Add BOOKS", 500, sid);
							reject(error);
						}
						else {
							for (const addedCategory of category) {
								db.query("INSERT INTO PROD_CAT (BID, CATID) VALUES (?, (SELECT CATID FROM CATEGORY WHERE NAME = ?));",
									[result.insertId, addedCategory], (addError, addResult) => {
										if (addError) {
											errorMsg(res, addError, "Internal Server Error - Add CATEGORY", 500, sid);
											reject(addError);
										}
		...
		const pid = await new Promise((resolve, reject) => {
			db.query('INSERT INTO INVENTORY (BID, SID ,COND, QTY, CP, SP, DISCOUNT, LANG) VALUES (?, ?, ?, ?, ?, ?, ?, ?);'
				, [bid, sid, condition, quantity, cp, sp, discount, language], (error, result) => {
					if (error) {
						errorMsg(res, error, "Internal Server Error - Add INVENTORY", 500, sid);
						reject(error);
					}
					else resolve(result.insertId);
				})
...
exports.orderDet = async (req, res) => {
	...
	try {
		...
		db.query("SELECT * FROM ORDERS WHERE OID = ?", [oid], (error, order) => {
			if (error) errorMsg(res, error, "Internal Server Error", 500, sid);
			if (result.length <= 0) errorMsg(res, null, "No Order ID Found", 404, sid);
			else {
				db.query("SELECT * FROM ORDER_ITEM WHERE OID = ?", [oid], async (error, items) => {
					if (error) errorMsg(res, error, "Internal Server Error", 500, sid);
					if (result.length <= 0) errorMsg(res, null, "No Order Items Found", 404, sid);
					else {
						// Fetch products for each item using another Promise.all
						const productQueries = items.map(orderItems => {
							return Promise.all(orderItems.map(item => {
